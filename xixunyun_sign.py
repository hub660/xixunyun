...
import chinese_calendar as calendar_china
import rsa
...
def ageing(moth):
    ...
    try:
        ...
        try:
            ...
            end_date = end_date.replace(day=calendar.monthrange(end_date.year, end_date.month)[1]) # 使用标准库的 calendar
    except ValueError:
...
def jiejiari():
    ...
    on_holiday, holiday_name = calendar_china.get_holiday_detail(today)
...
async def main():
    ...
    for user in users:
        ...
        if ageing_TF is False:
            bot_message += (f"{name} {account} 超出有效期")
            ...
            if len(user_record) > 3:
                ...
                if user_record_errow == "请求异常":
                    ...
                    break 
                elif user_record_errow['code'] == 40511 and user_record_errow['message'] == '登录超时' or user_record_errow['code'] == 40510:
                    ...
                    if usr_token_insp_len > 7:
                        ...
                        if len(user_record) > 3:
                            ...
                            bot_message_error_login += 1
                            bot_message = (f"{name} {account} 登录失败")
                            continue
                    else:
            ...
            if jiejiari() is True:
                ...
                #先看 mothxiu 类型 ，是数字还是什么
                if mothxiu.isdigit():
                    mothxiu = int(mothxiu)
                    if mothxiu != 0:
                        #最大月休次数 本月已上班次数 本月休息次数
                        print("【?】用户随机月休模式(节假日必须休)[今天上班]")
