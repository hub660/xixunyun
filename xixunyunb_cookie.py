...
"""
File: xixunyun_cookie.py(习讯云日周月报数据库)
Author: luoye
...
环境变量需求
school_id=学校代码,name=名字,phone=手机号,password=密码,moth=有效年月,word_long=工作地址经度,word_latit=工作地址纬度,word_name_guishu=工作地址的归属区,word_name=工作地址的名字,home_long=假期的经度,home_latit=假期的纬度,home_name_guishu=假期地址的归属区,after_word=延迟签到的时间,home_name=假期的地址名字,mothxiu=月休几天
推送消息变量
bot_message
'''
...
def validate_and_update_user(user, Env_cookies_value, data, file_name, bot_message, bot_message_sure, bot_message_error, from_config, version_config, platform_config):
    required_keys = [
        ...
        'standing', 'mothxiu', 'mac', 'after_word'
    ]
    ...
    ageing_TF = ageing(Env_cookies_value.get('moth'))
    if ageing_TF is False:
        bot_message += (f"{Env_cookies_value.get('name')} {Env_cookies_value.get('account')} 不在有效期内")
        bot_message_error += 1
        return bot_message, bot_message_sure, bot_message_error
    ...
    try:
        if existing_user:
            ...
            update_keys = [
                ...
                'home_name_guishu', 'after_word', 'standing', 'model'
            ]
            for key in update_keys:
                existing_user[key] = Env_cookies_value[key]

            # 有条件地更新报告设置，仅在环境变量中存在时才更新
            if 'day_report' in Env_cookies_value:
                existing_user['day_report'] = Env_cookies_value.get('day_report', existing_user.get('day_report', True))
            if 'week_report' in Env_cookies_value:
                existing_user['week_report'] = Env_cookies_value.get('week_report', existing_user.get('week_report', True))
            if 'month_report' in Env_cookies_value:
                existing_user['month_report'] = Env_cookies_value.get('month_report', existing_user.get('month_report', True))
            if 'report_mode' in Env_cookies_value:
                existing_user['report_mode'] = Env_cookies_value.get('report_mode', existing_user.get('report_mode', "1"))
            ...
            if (isinstance(new_token, list) or isinstance(new_token, tuple)) and len(new_token) > 7:
                ...
                new_user_data = {
                    ...
                    "after_word": str(Env_cookies_value['after_word']),
                    "standing": Env_cookies_value['standing'],
                    # 有条件地添加报告字段
                    "day_report": Env_cookies_value.get('day_report', True) if 'day_report' in Env_cookies_value else True,
                    "week_report": Env_cookies_value.get('week_report', True) if 'week_report' in Env_cookies_value else True,
                    "month_report": Env_cookies_value.get('month_report', True) if 'month_report' in Env_cookies_value else True,
                    "report_mode": Env_cookies_value.get('report_mode', "1") if 'report_mode' in Env_cookies_value else "1",
...
def Env_cookie_format(cookies, file_name, data, from_config, version_config, platform_config):
    ...
    for idx, cookie in enumerate(cookies, 1):
        ...
        try:
            ...
            Env_cookies_value['day_report'] = Env_cookies_value.get('day_report', 'true').lower() == 'true'
            Env_cookies_value['week_report'] = Env_cookies_value.get('week_report', 'true').lower() == 'true'
            Env_cookies_value['month_report'] = Env_cookies_value.get('month_report', 'true').lower() == 'true'
            Env_cookies_value['report_mode'] = (Env_cookies_value.get('report_mode', "1"))
        except ValueError:
            extracted = {
