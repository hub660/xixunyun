...
class Xixunyun_record():
    ...
    def get_record_aftr_work(self,all=None):
        current_date = datetime.now()
        formatted_date = current_date.strftime("%Y-%m").lstrip("0")
        # 如果月份部分以"0"开头，去掉这个"0"
        if formatted_date[5] == "0":
            ...
            if response.get('code') == 20000:
                ...
                if data_list:
                    ...
                    for record in data_list:
                        if record.get('sign_time_text') == today_date:
                            sign_time_text = record.get('sign_time_text')
                            remark_text = record.get('remark_text')
                            return sign_time_text, remark_text
            return None
        ...
        try:
            ...
            if result:
                today_date, today_word = result
                return today_date, today_word
            else:
                return "今天没有签到记录"
        except:
            return response_json
...
if __name__ == "__main__":
    #Token
    Token = 'e682eda0ecacf7ad01f377e7cf979931'
    #学校代码
    school_id = 842
    ...
    #查看今天的日期与情况
    usr_record_insp = Xixunyun_record(Token,school_id).get_record_aftr_work()
    # 返回值  最后一次打卡日期 与 打卡模式
    print((usr_record_insp))
